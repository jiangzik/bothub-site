...
<p class="term-output-note">Desktop / Android / iOS &nbsp;|&nbsp; 接多家 API &nbsp;|&nbsp; 登录可云同步</p>
...
你能用它做这些事:
  ...
  填上 API Key，就能用 OpenAI、Claude、Gemini 等几十家的模型。
  聊到一半想换个模型？直接切，不用新建对话。
  每个 Bot 可以绑不同的模型。

  模型不只能聊天。可以跑 bash 命令、读写文件、操控浏览器、
  截屏、联网搜索。需要更多工具的话，接个 MCP Server 就行。

  可以给每个 Bot 写性格和规则，还能把常用操作存成技能。
  技能支持从 GitHub 导入，也可以自己写。

  定时任务：设个 Cron 或者倒计时，Bot 到点自己跑。
  结果可以写进对话，也可以发到飞书、Telegram 或者 Webhook。
...
手机和电脑登录同一个账号就能连上，不用在同一个网络。
  连上之后可以在手机上开终端、跑命令、看电脑屏幕、直接点和打字。
  可以同时连多台电脑。
</pre>
</div>
